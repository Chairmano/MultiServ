...
async function handleLogin(e) {
  ...
  let valid = true;
  if (!email) { showError('loginEmail', 'Please enter your email.');
    valid = false; }
  if (!pass) { showError('loginPassword', 'Please enter your password.');
    valid = false; }
  if (!valid) return;
...
async function handleSignup(e) {
  ...
  let valid = true;
  if (!first) { showError('signupFirst', 'Required.');
    valid = false; }
  if (!last) { showError('signupLast', 'Required.');
    valid = false; }
  if (!username) { showError('signupUsername', 'Choose a username.');
    valid = false; }
  else if (username.length < 3) { showError('signupUsername', 'At least 3 characters.');
    valid = false; }
  
  if (!email) { showError('signupEmail', 'Enter your email address.');
    valid = false; }
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    showError('signupEmail', 'That doesn\'t look like a valid email.');
    valid = false;
  }
  
  if (!pass) { showError('signupPass', 'Create a password.');
    valid = false; }
  else if (pass.length < 8) { showError('signupPass', 'At least 8 characters required.');
    valid = false; }
  
  if (pass && confirm !== pass) { showError('signupConfirm', 'Passwords do not match.');
    valid = false; }
